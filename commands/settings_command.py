...
async def on_settings(client: discord.Client, interaction: discord.Interaction, setting: str = None, new_value: str = None) -> None:
    ...
    try:
        await database_scripts.check_create_settings(interaction.guild.id, db)
        ...
        match setting:
            ...
            case "level_up_message_channel_id":
                ...
                if not channel:
                    await interaction.response.send_message("Channel not found. Setting will be saved anyway.", ephemeral=True)
